...
main = hspec $ do
    ...
    describe "reset tests" $ do
        it "runs once with 1 test" $ do
            runTest [1..10] [(Reset, requireFilter even)] `shouldBe` filter even [1..10 :: Int]
            runTest [1..10] [(Reset, requireFilter even <|> requireFilter odd)] `shouldBe` [1..10 :: Int]
        it "runs once with 2 tests" $ do
            runTest [1..10] [(Reset, requireFilter even), (Reset, require 4)] `shouldBe` [2,4,4,6,8,10 :: Int]
            runTest [1..10] [(Reset, requireFilter even), (Reset, requireFilter odd)] `shouldBe` [1..10 :: Int]
